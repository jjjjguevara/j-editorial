...
# Paths updated by the 2026-09-04 program-major restructure. The original runner
# (Git blob 4ee5855c7b500c4941265cb007e59091e86998e4) read research/phase-2 and
# research/phase-3; only the input locations below changed. Because the report
# keys inputs by repository path, the experiment digest differs from the original
# run; the restructure packet's results/digest-equivalence.json shows the two
# digests agree once the path keys are mapped.
ROOT = Path(__file__).resolve().parents[4]
PROGRAMS = ROOT / "research/programs"
FIXTURES = [PROGRAMS / "event-state/fixtures/amnesia-notes-event-fluent.json",
            PROGRAMS / "prose/fixtures/portfolio-about-event-fluent.json"]
VALIDATORS = [PROGRAMS / "event-state/tools/validate_event_fluent_fixture.py",
              PROGRAMS / "prose/tools/validate_portfolio_prose_fixture.py"]
FRAGMENTS = PROGRAMS / "representation/fixtures/target-fragments.json"
...
def mutation_audit(fixtures: list[dict[str, Any]]) -> list[dict[str, Any]]:
    findings = []
    validators = [load_validator(path) for path in VALIDATORS]
...
def target_probes() -> list[dict[str, Any]]:
    manifest = json.loads(FRAGMENTS.read_text())
